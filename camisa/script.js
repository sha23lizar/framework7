...
window.onload = async function () {
  ...
  const moduloBeforeUnloadHandler = new ModuloBeforeUnloadHandler();
  var guardado = true
  const conexionServidor = new ConcexionServidor(id_user)
    ...
    btnEditarNombre.addEventListener('click', (e) => {
      ...
      swal("Como quieres que se llame tu Diseño", {
        ...
        if (value.trim().length > 0) {
          ...
          nombre = shirtDisign.nombre
        }
        tituloDisign.innerText = nombre
        shirtDisign.nombre = nombre
  ...
  btnSave.addEventListener('click', () => {
    ...
    // conexionServidor.subirPreviewDataUri(shirtDisign.preview)
    if (guardado) {
